...
func _refresh() -> void:
	...
	for item in definition.get("items", []):
		...
		button.custom_minimum_size = Vector2(286, 22)
...
func _build_ui() -> void:
	...
	var panel := Panel.new()
	panel.position = Vector2(140, 56)
	panel.size = Vector2(360, 248)
	...
	ornament.position = Vector2(180, 22)
	...
	var title := _label(panel, Vector2(20, 14), Vector2(320, 20), 14, Color("dfbd82"))
	title.z_index = 2
	title.text = "MERCADOR DA PRAÇA"
	title.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	_currency_label = _label(panel, Vector2(25, 39), Vector2(310, 16), 9, Color("f3e8d1"))
	_items = VBoxContainer.new()
	_items.position = Vector2(37, 63)
	_items.size = Vector2(286, 120)
	_items.add_theme_constant_override("separation", 2)
	panel.add_child(_items)
	_description = _label(panel, Vector2(28, 188), Vector2(304, 26), 9, Color("d6c6ab"))
	_description.autowrap_mode = TextServer.AUTOWRAP_WORD_SMART
	_feedback = _label(panel, Vector2(28, 218), Vector2(304, 15), 8, Color("dca76d"))
